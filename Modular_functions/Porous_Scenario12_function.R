...
# This function uses par[9] for leakage (Leak in Wendt et al. 2025) 
...
# run by GW_Porous_Baseline with the spinup climate dataset

#This GW_Porous function is for scenarios 1 and 2 that have trigger-level drought management practises (DMP)
...
GW_Porous_S12<- function(par,soilmoisture, D_GW,D_S, GSini,SPI,GS_B){
  ...
  for(t in 1:(length(o$Rch)-1)){
    o$Qb[t] =par[9]*o$GS[t]+par[10]*o$Rch[t]            # Generating baseflow (Qb)
